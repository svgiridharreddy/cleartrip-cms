...
class FlightsApprovalPending extends Component {
  ...
  approveRoute(id, table_name) {
    ...
    return new Promise(function(resolve) {
      axios
        .get(host() + "/route-approval", { params: data })
        .then(function(json) {
          let index = "";
          ...
          NotificationManager.success("successfully approved", "success", 1500);
  ...
  createTable(type, thead, data) {
    ...
    let columns = [];
    if (
      thead === "uniq_flight_schedule_routes" ||
      thead == "unique_flight_ticket_route"
    ) {
      columns = [
      ...
      thead == "uniq_flight_booking_overview" ||
      thead == "uniq_flight_booking_pnrweb" ||
      thead == "uniq_flight_booking_routes"
    ) {
      columns = [
        "Domain-Language-Section",
        "page_type",
    ...
    if (type == "columns") {
      ...
      columns.map(col => {
        if (col === "Domain-Language-Section") {
          tabObj["Domain-Language-Section"] =
            data["domain"] + "-" + data["section"] + "-" + data["language"];
        } else {
          tabObj[col] = data[col];
        }
      });
      tabObj["approve"] = (
        <MDBBtn
          ...
          rounded
          size="sm"
          onClick={() => this.approveRoute(data.id, thead)}
          disabled={data.is_approved ? true : false}
        >
          {data.is_approved ? "Approved" : "Approve"}
        </MDBBtn>
      );
      return tabObj;
    }
